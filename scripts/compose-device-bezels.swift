...
guard arguments.count >= 5 else {
    fputs("Usage: compose-device-bezels.swift <bezel.png> <screens-dir> <output-dir> <screen-name> [screen-name ...]\n", stderr)
    exit(1)
}
...
let screenNames = Array(arguments.dropFirst(4))
...
for name in screenNames {
    let candidates = ["simulator-\(name).png", "simulator-\(name).webp"]
    guard let sourceURL = candidates
        .map({ screensURL.appendingPathComponent($0) })
        .first(where: { fileManager.fileExists(atPath: $0.path) }),
          let screen = NSImage(contentsOf: sourceURL) else {
        fputs("Could not read simulator-\(name).png or .webp in \(screensURL.path)\n", stderr)
        exit(1)
    }
