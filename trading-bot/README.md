...
### 1. Бэктест и подбор параметров (первым делом)

```bash
python -m backtest.backtest --symbol BTC/USDT --days 365   # проверка стратегии
python -m backtest.optimize --symbol BTC/USDT --days 540   # walk-forward подбор параметров
```

Бэктест покажет доходность против buy&hold, просадку, winrate, profit factor.
Оптимизатор подбирает параметры на 70% истории и честно проверяет на
остальных 30% (out-of-sample) — доверяйте только OOS-цифрам.

Бот торгует в режиме `strategy.mode: auto`: определяет режим рынка по ADX
и включает трендовую стратегию (EMA-кроссовер) в тренде или mean-reversion
(разворот RSI из перепроданности) в боковике.
...
- **Отчёт о результатах:** `python -m bot.report` — капитал, PnL за 7/30 дней,
  статистика по стратегиям (консоль + Telegram). Удобно повесить на cron.
- **Funding-монитор:** `python -m monitor.funding` — оповещает, когда кэрри
  «лонг спот + шорт перп» даёт привлекательную годовую ставку (входит в
  docker-compose).
