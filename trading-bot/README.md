...
## Самоулучшение со временем

Система адаптируется к рынку тремя механизмами (все включены в docker-compose):

1. **Адаптивный риск** (`adaptive` в конфиге): стратегия с хорошим profit
   factor в последних 20 сделках получает до 1.25x капитала, сливающая —
   0.25x (не ноль — сможет реабилитироваться, когда рынок снова станет её).
2. **Автоперенастройка** (`python -m backtest.retune --apply --loop`, сервис
   `retune`): раз в неделю walk-forward подбор параметров на свежих данных.
   Новые параметры применяются **только если** на out-of-sample данных они
   лучше текущих минимум на 2 п.п. и не хуже по просадке. Работающие боты
   подхватывают их на лету, история решений — в `data/retune_log.jsonl`.
3. **Ротация умных кошельков** (`smartmoney.rescore_hours`): раз в сутки
   рейтинг пересчитывается — кошельки, переставшие зарабатывать, вылетают,
   новые лидеры добавляются. Смена состава не порождает ложных сигналов.
